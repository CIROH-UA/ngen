...
NetCDFPerFeatureDataProvider::NetCDFPerFeatureDataProvider(std::string input_path, time_t sim_start, time_t sim_end, utils::StreamHandler log_s, bool enable_cache)
    : log_stream(log_s)
...
double NetCDFPerFeatureDataProvider::get_value(const CatchmentAggrDataSelector& selector, ReSampleMethod m) 
{
    ...
    // Save the original NetCDF catchment index before id_pos may be rebased
    auto nc_cat_idx = id_pos.at(selector.get_id());
...
if (!enable_cache)
{
    // Cacheless mode: read only the requested catchment and time range
    // directly from NetCDF.
    const std::size_t read_len = c_idx2 - c_idx1 + 1;

    std::vector<double> raw_values(read_len);

    start.clear();
    start.push_back(nc_cat_idx);
    start.push_back(c_idx1);

    count.clear();
    count.push_back(1);
    count.push_back(read_len);

    ncvar.getVar(start, count, raw_values.data());
    ...
    switch (m)
    {
        ...
        {
            double scale_factor =
                (selector.get_duration_secs() > time_stride)
                    ? (time_stride / selector.get_duration_secs())
                    : (1.0 / (a + b));
...
else
{
    ...
    // pages spanned by [c_idx1, c_idx2]
    std::size_t n_page_accesses =
        cache::page_p_idx(c_idx2, cache_line_size) - p_idx + 1;
    ...
    for (size_t i = 0; i < n_page_accesses; i++)
    {
        ...
        std::size_t page_c_idx =
            cache::page_p_idx_to_c_idx(ith_p_idx, cache_line_size);

        std::size_t page_cache_line_size =
            cache::page_cache_line_size(
                page_c_idx,
                time_vals.size(),
                cache_line_size);

        std::string key =
            ncvar.getName() + "|" + std::to_string(page_c_idx);
        ...
        {
            cached = std::make_shared<std::vector<double>>(
                get_ids().size() * page_cache_line_size);
